...
searchurl='/groups/%s/projects?private_token=%s'%(args.source_namespace,gitlab_token)
print('trying group via '+ gitlab_url + searchurl)

res = s.get(gitlab_url + searchurl)
if res.status_code == 404 or len(json.loads(res.text)) < 1:
    searchurl='/users/%s/projects?private_token=%s'%(args.source_namespace,gitlab_token)
    print('group url did not work.. searching user projects ')
# via ' + gitlab_url + searchurl)
    res = s.get(gitlab_url + searchurl)
#    print(str(res.text))
...
while not finished:
    ...
    res = s.get(gitlab_url + searchurl + '&page=%s'%(page_id))
...
for i in range(len(filtered_projects)):
    ...
    # Create repo 
    if args.add_to_private:
        print('adding to private via '+gogs_url+'/user/repos')
        ...
        print('adding to org via '+gogs_url+'/org/%s/repos'%args.add_to_organization)
